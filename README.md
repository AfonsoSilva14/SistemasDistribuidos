...
SENSOR ──publica──►  sensores_exchange  ──subscreve──►  GATEWAY
                                  ...
                                  RPC (HTTP/JSON) ────────►│ PreProcessingService (C#, :7001)
                                                       ...
                                                       SERVIDOR  (TCP :5000)
                                                          │
                                  RPC (HTTP/JSON) ────────►│ AnalysisService (Python, :7002)
...
| **AnalysisService** | **Python** (stdlib) | RPC — classificação de risco + análise estatística (média, mín, máx, desvio-padrão, tendência) |
| **Consola** | C# / SQLite | Interface CLI: consultar medições, **pedir análises parametrizadas**, ver histórico |

> O **AnalysisService em Python** e o pré-processamento com conversão de
> escalas são fatores de valorização (linguagens/tecnologias distintas).
...
Abrir um terminal por componente:

```bash
# 1. Serviço de análise (Python)
cd SistemaMonitorizacao/AnalysisService && python3 analysis_service.py
